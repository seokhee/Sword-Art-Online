...
class MySys(bgui.System):
	def __init__(self):
		...
		self.main_menu = bgui.Image(self.main_frame, 'main_menu', '../GUI/GUI_Main_Manu.png', size=[0.8,1.1], pos=[-0.1, -0.1])
		self.main_menu.colors = [[0.9,0.9,0.9,0.5]] * 4
		
		# Show Sub Menu
		self.sub_menu = bgui.Frame(self.main_frame, 'sub_menu', border=0, size=[0.3,1.5], pos=[0.7, -0.95], options = bgui.BGUI_DEFAULT|bgui.BGUI_CACHE) # 1:-0.9 2:-0.6  3:-0.3  4:0  5:0.3
	...
	def moveMenu(self):
		global menu_Status
		if menu_Status == 1:
			self.sub_menu.position = [0.7, 0.25]
		elif menu_Status == 2:
			self.sub_menu.position = [0.7, -0.05]
		elif menu_Status == 3:
			self.sub_menu.position = [0.7, -0.35]
		elif menu_Status == 4:
			self.sub_menu.position = [0.7, -0.65]
		elif menu_Status == 5:
			self.sub_menu.position = [0.7, -0.95]
