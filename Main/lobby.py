...
class MySys(bgui.System):
	def __init__(self):
		...
		self.lbl = bgui.Label(self.note, 'lblconnect', text="|-| Connections |-|",pt_size = 35, pos=[0.02,0.93], options = bgui.BGUI_DEFAULT)
		self.lbl.color = [0,0,0,1]
		items = [nick]+['Kirito','Kaira','Korui','YoungZaCharac','Hello','World','World of Sao','Dev','Build','df','ddd','wer','dd','Asuna']
		i1 = len(items) - 14
		if i1 < 1:
			i1 = 'no more'
		item = items[0:13] + ['... And '+str(i1)+' players.']
		...
		self.buttons = bgui.FrameButton(self.foot, 'buttons', text='Send',pt_size = 30, size=[0.085, 0.8], pos=[0.91, 0.1],options = bgui.BGUI_DEFAULT)
		...
		self.input2 = bgui.TextInput(self.foot, 'input2', "",pt_size = 35, size=[1, 0.95], pos=[0.03, 0.01],
			input_options = bgui.BGUI_INPUT_SELECT_ALL, options = bgui.BGUI_DEFAULT)
		self.input2.color = [0.5,0.5,0.5,0.8]
		
		if svrstat == 0:
			self.chattext = bgui.TextBlock(self.listc, 'chatting',text = chat, pt_size = 35, color = [0.3,0.85,0.3,1], size = [0.98, 0.98], pos = [0.02,0])
			self.chattext.text = tmp1 + 'System ::> Connect to server' + tmp1
		elif svrstat == 1:
			self.chattext = bgui.TextBlock(self.listc, 'chatting',text = chat, pt_size = 35, color = [1,0.6,0.2,1], size = [0.98, 0.98], pos = [0.02,0])

		#creat next button
		self.button = bgui.FrameButton(self, 'button', text='|| Join\n\n|| Server',pt_size = 40, size=[0.14, 0.13], pos=[0.845, 0.022],options = bgui.BGUI_DEFAULT)
		self.button.on_click = self.multi
		self.button.on_hover = self.hover_sound
		self.button.on_mouse_exit = self.setting
		
		self.button = bgui.FrameButton(self, 'button1', text='|| Story\n\n|| Mode',pt_size = 40, size=[0.14, 0.13], pos=[0.68, 0.022],options = bgui.BGUI_DEFAULT)
		...
		self.lbl2 = bgui.Label(self, 'lab2', text="Hello, "+str(data[0])+".",pt_size = 55, pos=[0.7015,0.927], options = bgui.BGUI_DEFAULT)
		self.lbl2.color = [0.1,0.1,0.1,0.6]
